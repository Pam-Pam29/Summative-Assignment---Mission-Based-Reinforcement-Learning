...
def render_frame(env):
 ...
 ep_val = font_large.render(f"{env.episode_reward:.1f}", True,
 ...
 action_icon = ACTION_ICONS[env.last_action]

 # Big action bubble
 pygame.draw.rect(surface, action_color, (315, 125, 270, 80), border_radius=16)
 act_surf = font_large.render(f"{action_icon} {action_name}", True, WHITE)
 ...
 txt = font_medium.render(f" {ACTION_ICONS[i]} {aname}", True,
 ...
 # Draw reward bar chart (steps 0-9)
 bar_w = 20
 bar_gap = 5
 base_y = 380
 max_bar = 120
 ...
 # Placeholder bars — highlight current step
