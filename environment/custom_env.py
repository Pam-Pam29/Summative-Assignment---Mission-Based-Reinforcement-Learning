...
class SistaHealthEnv(gym.Env):
    TOPICS = [
        'FGM Complications', 'VVF Causes', 'Cultural Barriers',
    ...
    def __init__(self, render_mode=None):
        ...
        self.observation_space = spaces.Box(
            ...
            high=np.array([2, 1, 8, 2, 9], dtype=np.float32),
            dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        if action == 0:
            if literacy == 2:
                reward, feedback = 10, 'Text excellent for high literacy. +10'
            elif literacy == 1:
                reward, feedback = 5, 'Text good for medium literacy. +5'
            else:
                reward, feedback = -4, 'Text poor for low literacy. -4'

        elif action == 1:
            if literacy == 0:
                reward, feedback = 14, 'Voice perfect for low literacy. +14'
                if language == 2:
                    reward, feedback = 16, 'Voice+Pidgin for low literacy. +16'
            elif literacy == 1:
                reward, feedback = 7, 'Voice good for medium literacy. +7'
                if language == 2:
                    reward, feedback = 9, 'Voice+Pidgin for medium literacy. +9'
            else:
                reward, feedback = 2, 'Voice acceptable for high literacy. +2'

        elif action == 2:
            reward, feedback = 3, 'Resource link shared. +3'

        elif action == 3:
            if step < 3:
                reward, feedback = 4, 'Clarification helpful early. +4'
            else:
                reward, feedback = -2, 'Clarification stalling. -2'

        self.step_count += 1
        self.state[4] = min(self.step_count, 9)
